...
(memory-mapped file) and on CUDA (blocks uploaded once). `PQ2_0` linears stay packed on CPU. Other
devices decode to dense weights at load and unfold the fold once.
