...
| `mistralrs_grammar_ff_splice_drops_total` | counter | `reason` | Staged spans discarded before they reached a decode window. `reason` is `batch_shape` (another sequence in the same step staged a span of a different length, or none at all), `preemption` (the sequence was preempted before its span was fed), or `realloc` (the sequence's token list was rebuilt, invalidating the staged span). |
