...
Use `mistralrs bench -m <model>` for single-request kernel measurements. For server tuning, replay
...
If you enable [grammar fast-forward](/guides/serve/structured-output/#grammar-fast-forward), be aware it currently sees close to the flag-off baseline under concurrent grammar-constrained load; higher `--max-seqs` does not help it the way it helps everything else in this section.
